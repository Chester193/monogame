...
namespace PG2D_2020_Dzienni_FD_Projekt
{
    static class Camera
    {
        ...
        public static void Initialize(float zoomLevel = 1.0f)
        {
            zoom = zoomLevel;
            rotation = 0.0f;

            //Start the camera at the center of the screen:
            position = new Vector2(ResolutionManager.VirtualWidth / 2, ResolutionManager.VirtualHeight / 2);
        }
        ...
        private static void CalculateMatrixAndRectangle()
        {
            ...
            transformMatrix = transformMatrix * ResolutionManager.getTransformMatrix();
        ...
        private static Rectangle VisibleArea()
        {
            ...
            Vector2 tr = Vector2.Transform(new Vector2(ResolutionManager.VirtualWidth, 0), inverseViewMatrix);
            Vector2 bl = Vector2.Transform(new Vector2(0, ResolutionManager.VirtualHeight), inverseViewMatrix);
            Vector2 br = Vector2.Transform(new Vector2(ResolutionManager.VirtualWidth, ResolutionManager.VirtualHeight), inverseViewMatrix);
            ...
            return new Rectangle((int)min.X, (int)min.Y, (int)(ResolutionManager.VirtualWidth / zoom), (int)(ResolutionManager.VirtualHeight / zoom));
        }
